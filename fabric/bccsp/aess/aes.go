...
// AESCBCPKCS7EncryptWithRand AESCBCPKCS7Encrypt combines CBC encryption and PKCS7 padding using as prng the passed to the function
...
// AESCBCPKCS7EncryptWithIV AESCBCPKCS7Encrypt combines CBC encryption and PKCS7 padding, the IV used is the one passed to the function
...
func (e *Aescbcpkcs7Encryptor) Encrypt(k bccsp.Key, plaintext []byte, opts bccsp.EncrypterOpts) ([]byte, error) {
	...
	case *AESCBCPKCS7ModeOpts:
		...
		if len(o.IV) != 0 {
			...
			return AESCBCPKCS7EncryptWithIV(o.IV, k.(*AesPrivateKey).PrivateKey, plaintext)
		} else if o.PRNG != nil {
			// Encrypt with PRNG
			return AESCBCPKCS7EncryptWithRand(o.PRNG, k.(*AesPrivateKey).PrivateKey, plaintext)
		}
		// AES in CBC mode with PKCS7 padding
		return AESCBCPKCS7Encrypt(k.(*AesPrivateKey).PrivateKey, plaintext)
	case AESCBCPKCS7ModeOpts:
...
func (*Aescbcpkcs7Decryptor) Decrypt(k bccsp.Key, ciphertext []byte, opts bccsp.DecrypterOpts) ([]byte, error) {
	...
	case *AESCBCPKCS7ModeOpts, AESCBCPKCS7ModeOpts:
		// AES in CBC mode with PKCS7 padding
		return AESCBCPKCS7Decrypt(k.(*AesPrivateKey).PrivateKey, ciphertext)
	default:
		return nil, fmt.Errorf("Mode not recognized [%s]", opts)
	}
}
