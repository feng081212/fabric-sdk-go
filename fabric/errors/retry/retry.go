...
// clients in the SDK:
// https://godoc.org/fabric-sdk/pkg/client/channel#WithRetry
// https://godoc.org/fabric-sdk/pkg/client/resmgmt#WithRetry
package retry
...
type Opts struct {
	...
	RetryableCodes map[status2.Group][]status2.Code
}
...
func (i *impl) isRetryable(g status2.Group, c int32) bool {
	for group, codes := range i.opts.RetryableCodes {
		if g != group {
			continue
		}
		for _, code := range codes {
			if status2.Code(c) == code {
				return true
			}
		}
	}
