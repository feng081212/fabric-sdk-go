...
var GroupName = map[int32]string{
	...
	7:  "Endorser ClientContext Status",
	8:  "Orderer ClientContext Status",
	9:  "ClientContext Status",
	10: "Chaincode status",
	11: "Discovery status",
	12: "Test status",
}
...
func FromError(err error) (s *Status, ok bool) {
	...
	if m, ok := unwrappedErr.(multi.Errors); ok {
		return New(ClientStatus, MultipleErrors.ToInt32(), m.Error()), true
	}
...
func New(group Group, code int32, msg string) *Status {
	return &Status{Group: group, Code: code, Message: msg}
}
...
func NewFromGRPCStatus(s *grpcstatus.Status) *Status {
	...
	return &Status{Group: GRPCTransportStatus, Code: s.Proto().Code, Message: s.Message()}
}

func NewFromGRPCError(e error) *Status {
	rpcStatus, ok := grpcstatus.FromError(e)
	if ok {
		return NewFromGRPCStatus(rpcStatus)
	}
	return nil
}
